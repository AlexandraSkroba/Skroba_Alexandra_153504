...
int main() {
	...
	for (int i = 1; i < n - 1; i++) {
		for (int j = 1; j < m - 1; j++) {
			if (arr[i][j] == '*') {
				if (arr[i][j + 1] == ' ') arr[i][j + 1] = number;
				else if (arr[i][j + 1] != '#' && arr[i][j + 1] != '*') arr[i][j + 1] += 1;

				if (arr[i][j - 1] == ' ') arr[i][j - 1] = number;
				else if (arr[i][j - 1] != '#' && arr[i][j - 1] != '*') arr[i][j - 1] += 1;

				if (arr[i + 1][j] == ' ') arr[i + 1][j] = number;
				else if (arr[i + 1][j] != '#' && arr[i + 1][j] != '*') arr[i + 1][j] += 1;

				if (arr[i - 1][j] == ' ') arr[i - 1][j] = number;
				else if (arr[i - 1][j] != '#' && arr[i - 1][j] != '*') arr[i - 1][j] += 1;

				if (arr[i - 1][j - 1] == ' ') arr[i - 1][j - 1] = number;
				else if (arr[i - 1][j - 1] != '#' && arr[i - 1][j - 1] != '*') arr[i - 1][j - 1] += 1;
